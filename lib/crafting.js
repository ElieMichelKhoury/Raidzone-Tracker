/**
 * Recursively resolve how many raw materials are needed
 * to craft `qty` of `materialId`.
 *
 * @param {string} materialId
 * @param {number} qty
 * @param {Array}  recipes  — array of recipe objects with .outputId, .outputQty, .ingredients [{materialId, qty}]
 * @param {number} depth    — internal, prevents infinite loops
 * @returns {{ raws: Object, steps: Array }}
 */
export function resolveRaws(materialId, qty, recipes, depth = 0) {
  ...
  for (const ing of recipe.ingredients) {
    ...
    for (const [k, v] of Object.entries(sub.raws)) {
      raws[k] = (raws[k] || 0) + v;
    }
