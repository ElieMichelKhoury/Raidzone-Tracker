-- ============================================================
-- RAIDZONE CRAFTING TRACKER — Supabase Schema
-- Run this in your Supabase SQL editor (Dashboard > SQL Editor)
-- ============================================================

-- USERS (extended profile, linked to Supabase auth via steam)
...
create table if not exists public.clans (
  ...
  tag         text not null,           -- short 2-5 char tag e.g. [RZ]
...
-- MATERIALS (global, admin-managed)
...
-- PERSONAL INVENTORY (per user, per material)
...
-- CLAN INVENTORY (shared, per clan, per material)
...
-- Clans: anyone can read, members can update, owner can delete
...
create policy "clans_update_owner" on public.clans for update using (
  auth.uid() = owner_id
);
create policy "clans_delete_owner" on public.clans for delete using (auth.uid() = owner_id);

-- Clan members: read all, insert self, delete self or owner
...
-- Materials & Recipes: public read, admin write (handled server-side)
...
-- Materials write: admin only (use service role key server-side)
