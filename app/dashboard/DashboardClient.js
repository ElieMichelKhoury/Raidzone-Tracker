...
function MatIcon({ icon, name, size }) {
  const px = size || 20;
  if (icon && (icon.startsWith('/') || icon.startsWith('http'))) {
    return <img src={icon} alt={name} style={{ width: px, height: px, objectFit: 'contain', borderRadius: 3 }} />;
  }
  return <span style={{ fontSize: px }}>{icon || '📦'}</span>;
}
...
function DashboardOverview({ user, materials, recipes, clan }) {
  ...
  const craftable = recipes.filter(r =>
    r.ingredients.every(i => { const m = materials.find(x => x.id === i.material_id); return m && m.owned >= i.qty; })
  ).length;
          ...
          <div className="card-b">
            {empty.length === 0
              ...
              : <div className="col gap8">{empty.slice(0, 9).map(m => (
                  <div key={m.id} className="row jbet">
                    <span className="row gap8"><MatIcon icon={m.icon} name={m.name} size={18} /> {m.name}</span>
                    <span className="red bold mono">0</span>
                  </div>
                ))}</div>
            }
          </div>
        ...
        <div className="card-b col gap14">
          <div className="field">
            ...
            <select className="inp" value={recId} onChange={e => { setRecId(e.target.value); setResult(null); }}>
              ...
              {recipes.map(r => { const m = getMat(r.output_id); return <option key={r.id} value={r.id}>{m?.name || r.name}</option>; })}
            </select>
          </div>
          <div className="field">
            ...
            {selRec.ingredients.map(i => {
              ...
              return (
                <div key={i.material_id} className="row jbet mb8">
                  <span className="row gap8"><MatIcon icon={m?.icon} name={m?.name} size={18} /> {m?.name || i.material_id}</span>
                ...
                return (
                  <div key={id} className="raw-item">
                    <div className="row gap8">
                      <MatIcon icon={mat?.icon} name={mat?.name} size={24} />
                      <div>
        ...
        <div className="tw">
          <table>
            ...
            <tbody>
              {list.map(m => (
                <tr key={m.id}>
                  <td><div className="row gap8"><MatIcon icon={m.icon} name={m.name} size={22} /><strong>{m.name}</strong></div></td>
                  <td className="muted">{m.category}</td>
                  <td><RarityTag r={m.rarity} /></td>
                  <td>
                    <div className="row gap8">
                      <input className="inp" type="number" style={{ width: 75 }} value={m.owned}
      ...
      <div className="g2" style={{ alignItems: 'start' }}>
        <div className="card">
          ...
          <div className="card-b col gap14">
            ...
            <div className="field"><label>TAG (2-5 chars)</label><input className="inp" maxLength={5} value={createForm.tag} onChange={e => setCreateForm(f => ({ ...f, tag: e.target.value }))} /></div>
        ...
        <div className="g2" style={{ flexWrap: 'wrap' }}>
          {(clanData.members || []).map(m => (
            <div key={m.user_id} className="member-card">
              <div className="member-avatar">{m.profiles?.avatar_url ? <img src={m.profiles.avatar_url} alt={m.profiles.username} /> : null}</div>
              <div><div className="bold">{m.profiles?.username || 'Unknown'}</div><div className="muted text-sm">{m.role}</div></div>
            </div>
          ))}
        </div>
      )}
                ...
                {materials.map(m => {
                  ...
                  return (
                    <tr key={m.id}>
                      <td><div className="row gap8"><MatIcon icon={m.icon} name={m.name} size={22} /><strong>{m.name}</strong></div></td>
                      <td className="muted">{m.category}</td>
                      <td><input className="inp" type="number" style={{ width: 80 }} value={owned} onChange={e => updateClanInv(m.id, Math.max(0, parseInt(e.target.value) || 0))} /></td>
                      <td>
                        <div className="row gap4">
          ...
          <table>
            ...
            <tbody>
              {list.map(r => {
                ...
                return [
                  <tr key={r.id}>
                    <td><div className="row gap8"><MatIcon icon={out?.icon} name={out?.name} size={20} /><strong>{r.name}</strong></div></td>
                          ...
                          {r.ingredients.map(ing => {
                            ...
                            return (
                              <div key={ing.material_id} className="row gap8" style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 4, padding: '5px 10px' }}>
                                <MatIcon icon={m?.icon} name={m?.name} size={18} />
                                <span className="bold">{m?.name || ing.material_id}</span>
                                <span className="mono acc">×{ing.qty}</span>
                              </div>
                            );
                          })}
                        </div>
...
function MatModal({ mat, onSave, onClose }) {
  const [f, setF] = useState(mat || { id: '', name: '', category: 'Raw', rarity: 'Common', icon: '📦', owned: 0 });
      ...
      <div className="modal">
        ...
        <div className="modal-b">
          <div className="g2">
            ...
            <div className="field">
              <label>ICON — emoji or /image.png</label>
              <input className="inp" value={f.icon} onChange={e => s('icon', e.target.value)} placeholder="🟡 or /sulfur.png" />
              <div className="muted" style={{ fontSize: 11, marginTop: 4 }}>Preview: <MatIcon icon={f.icon} name={f.name} size={24} /></div>
            </div>
          </div>
          <div className="g2">
  ...
  return (
    <div className="overlay">
      <div className="modal" style={{ width: 560 }}>
        ...
        <div className="modal-f"><button className="btn btn-s" onClick={onClose}>Cancel</button><button className="btn btn-p" onClick={save}>💾 Save Recipe</button></div>
      </div>
    </div>
  );
}
...
function AdminPanel({ materials, setMaterials, recipes, setRecipes }) {
  ...
  async function saveMaterial(m) {
    await fetch('/api/recipes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'upsert_material', ...m }) });
    setMaterials(ms => { const i = ms.findIndex(x => x.id === m.id); if (i >= 0) { const n = [...ms]; n[i] = { ...n[i], ...m }; return n; } return [...ms, { ...m, owned: 0 }]; });
    setMatModal(null);
  }

  async function deleteMaterial(id) {
    if (!confirm('Delete material?')) return;
    await fetch('/api/recipes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete_material', id }) });
    setMaterials(ms => ms.filter(m => m.id !== id));
  }

  async function saveRecipe(r) {
    await fetch('/api/recipes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'upsert_recipe', ...r }) });
    setRecipes(rs => { const i = rs.findIndex(x => x.id === r.id); if (i >= 0) { const n = [...rs]; n[i] = r; return n; } return [...rs, r]; });
    setRecModal(null);
  }

  async function deleteRecipe(id) {
    if (!confirm('Delete recipe?')) return;
    await fetch('/api/recipes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'delete_recipe', id }) });
  ...
  return (
    <div className="col gap20">
      ...
      <div className="tabs">
        {[['materials', '📦 Materials'], ['recipes', '⚙️ Recipes'], ['activity', '👥 Player Activity']].map(([k, l]) => (
          <div key={k} className={`tab ${tab === k ? 'active' : ''}`} onClick={() => { setTab(k); if (k === 'activity' && !activity) loadActivity(); }}>{l}</div>
        ))}
      </div>
        ...
        <div className="col gap12">
          ...
          <div className="card"><div className="tw"><table>
            ...
            <tbody>{materials.map(m => (
              <tr key={m.id}>
                <td><MatIcon icon={m.icon} name={m.name} size={24} /></td>
      ...
      {tab === 'activity' && (
        <div className="col gap16">
          <div className="row jbet">
            <span className="muted">Login history and active sessions</span>
            <button className="btn btn-s btn-sm" onClick={loadActivity} style={{ opacity: loadingActivity ? .5 : 1 }}>
          ...
          {!activity && !loadingActivity && (
            <div style={{ textAlign: 'center', padding: 40 }}>
              <button className="btn btn-p" onClick={loadActivity}>Load Activity Data</button>
            </div>
          )}
          ...
          {activity && (
            <div className="col gap16">
              <div className="card">
                <div className="card-h">
                  <span className="card-t">🟢 CURRENTLY ACTIVE SESSIONS</span>
                  <span className="badge">{activity.activeSessions.length}</span>
                </div>
                <div className="tw">
                  <table>
                    <thead><tr><th>PLAYER</th><th>STEAM ID</th><th>SESSION EXPIRES</th></tr></thead>
                    <tbody>
                      {activity.activeSessions.length === 0 && (
                        <tr><td colSpan={3} className="muted" style={{ textAlign: 'center', padding: 20 }}>No active sessions right now</td></tr>
                      )}
                      {activity.activeSessions.map((s, i) => (
                        <tr key={i}>
                          <td>
                            <div className="row gap8">
                              {s.profiles?.avatar_url && <img src={s.profiles.avatar_url} style={{ width: 28, height: 28, borderRadius: 4 }} alt="" />}
                              <strong>{s.profiles?.username || 'Unknown'}</strong>
                            </div>
                          </td>
                          <td className="mono muted" style={{ fontSize: 11 }}>{s.profiles?.steam_id}</td>
                          <td className="mono" style={{ fontSize: 12 }}>{new Date(s.expires_at).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="card">
                <div className="card-h">
                  <span className="card-t">📋 LOGIN HISTORY</span>
                  <span className="badge">{activity.logs.length}</span>
                </div>
                <div className="tw">
                  <table>
                    <thead><tr><th>PLAYER</th><th>STEAM ID</th><th>LOGGED IN AT</th></tr></thead>
                    <tbody>
                      {activity.logs.length === 0 && (
                        <tr><td colSpan={3} className="muted" style={{ textAlign: 'center', padding: 20 }}>No login history yet</td></tr>
                      )}
                      {activity.logs.map((log, i) => (
                        <tr key={i}>
                          <td>
                            <div className="row gap8">
                              {log.profiles?.avatar_url && <img src={log.profiles.avatar_url} style={{ width: 28, height: 28, borderRadius: 4 }} alt="" />}
                              <strong>{log.profiles?.username || 'Unknown'}</strong>
                            </div>
                          </td>
                          <td className="mono muted" style={{ fontSize: 11 }}>{log.profiles?.steam_id}</td>
                          <td className="mono" style={{ fontSize: 12 }}>{new Date(log.logged_in_at).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
    ...
    <div className="app">
      ...
      <div className="main">
        <div className="topbar">
          <div>
            <div className="topbar-t">{PAGES.find(p => p.id === page)?.label}</div>
